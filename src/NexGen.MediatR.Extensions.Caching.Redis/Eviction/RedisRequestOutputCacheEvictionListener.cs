...
/// <summary>
/// Subscribes to Redis Pub/Sub eviction notifications and applies them locally.
/// </summary>
internal sealed class RedisRequestOutputCacheEvictionListener : BackgroundService
{
    private readonly IConnectionMultiplexer _connection;
    private readonly RedisEvictionOptions _options;
    private readonly RequestOutputCacheEvictionNode _node;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly ILogger<RedisRequestOutputCacheEvictionListener> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="RedisRequestOutputCacheEvictionListener"/> class.
    /// </summary>
    public RedisRequestOutputCacheEvictionListener(
        ...
        IServiceProvider serviceProvider)
    {
        ...
        _logger = serviceProvider.GetService<ILogger<RedisRequestOutputCacheEvictionListener>>()
            ?? NullLogger<RedisRequestOutputCacheEvictionListener>.Instance;
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            await foreach (var notification in mailbox.Reader.ReadAllAsync(stoppingToken).ConfigureAwait(false))
            {
                ...
                {
                    _logger.LogWarning(ex, "Failed to apply Redis cache eviction notification for tags {Tags}.", notification.Tags);
                }
            }
        }
        finally
        {
            ...
            {
                _logger.LogDebug(ex, "Failed to unsubscribe from Redis eviction channel {Channel}.", _options.Channel);
            }
